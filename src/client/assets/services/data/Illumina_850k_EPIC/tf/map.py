...
def fimo(dbs):
    ...
    for k in dbs.keys():
        for p in paddings:
            sys.stderr.write("searching against [%s] with padding [%d]\n" % (k, p))
            ...
            job_name = "eforge_profile_850k_probe_tf_map_%s_%d" % (k, p)
            cluster_script = os.path.join(cwd, "map_cluster.sh")
            cmd = "sbatch --parsable --mem-per-cpu=%d --partition=\"%s\" --workdir=\"%s\" --job-name=\"%s\" --wrap=\"%s %s %s %s %s %d\"" % (mem_per_cpu, partition, work_dir, job_name, cluster_script, fq_probe_fn, fimo_fn, dest_starch_fn, dest_tabix_fn, p)
            try:
