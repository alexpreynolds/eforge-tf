...
//    console.log("plot - updatePlot()", this.timeStampInMs);
    ...
    const rightTFEdge = parseInt(windowRangeStopAdjusted - windowRangeStartAdjusted) - padding;
    ...
    const xAxisBottom = d3.axisBottom(xAxisScale)
      ...
      .tickFormat(function(d,i) {
        ...
        if (reformat < leftTFEdge) {
          ...
          reformat -= (parseInt(windowRangeStopAdjusted - windowRangeStartAdjusted) - padding);
    ...
    const truePlotXMidpoint = 10 + xAxisTickPositions[padding] + parseFloat((xAxisTickPositions[xAxisTickPositions.length-1] - 2*xAxisTickPositions[padding-1])/2.0);
    ...
    d3.select(node)
      ...
      .attr('y', parseFloat(truePlotTop) - AppConst.settings.style.generic.axisTopPadding)
      .attr('height', truePlotHeight + AppConst.settings.style.generic.axisTopPadding)
      .attr('width', xAxisTickPositions[rightTFEdge] - xAxisTickPositions[leftTFEdge]);
    ...
    d3.select(node)
      .append("g")
    	.append("line")
      .attr("class", "line")
      .attr("x1", xAxisTickPositions[leftTFEdge]).attr("y1", truePlotBottom)
      .attr("x2", xAxisTickPositions[leftTFEdge]).attr("y2", parseFloat(truePlotTop) - AppConst.settings.style.generic.probePositionMarkerPadding)
    	.attr("stroke", AppConst.settings.style.color.probePositionMarkerLineStroke).attr("stroke-dasharray", "4, 4");
    d3.select(node)
      .append("g")
    	.append("line")
      .attr("class", "line")
      .attr("x1", xAxisTickPositions[rightTFEdge]).attr("y1", truePlotBottom)
      .attr("x2", xAxisTickPositions[rightTFEdge]).attr("y2", parseFloat(truePlotTop) - AppConst.settings.style.generic.probePositionMarkerPadding)
    	.attr("stroke", AppConst.settings.style.color.probePositionMarkerLineStroke).attr("stroke-dasharray", "4, 4");
    ...
    var focus = d3.select(node)
      ...
      .attr("class", "focus")
      .style("display", "none");
    var focusInnerCircle = focus.append("circle")
      .attr("fill", AppConst.settings.style.color.signalHighlightFocusCircleFill)
      .attr("r", 10)
      .style("display", "none");
    ...
    var queryProbeMatchNode = d3.select(node)
      .append("g")
      .attr("class", "probeMatch");
      
    var queryProbeMatchMarkers = queryProbeMatchNode.selectAll("probeMatch")
      ...
      .attr("cx", (d) => { 
    	...
    	.attr("cy", (d) => {
      	var yPosn = this.state.clientHeight - yScale(signal[d.position]) - this.state.clientMargin.bottom + this.state.clientMargin.top - 1;
      	return yPosn;
      })
    	.attr("r", 8)
    	.attr("fill", AppConst.settings.style.color.signalPrimaryProbeMarkerCircleFill)
    	.on("mouseenter", (d) => {
      	var xPosn = xAxisTickPositions[d.position];
      	var yPosn = this.state.clientHeight - yScale(signal[d.position]) - this.state.clientMargin.bottom + this.state.clientMargin.top - 1;
        var offsetIdx = Math.floor(((xPosn-truePlotLeft)/(truePlotLRDiff))*(signal.length));
        var base = windowRangeStartAdjusted + parseInt(offsetIdx); 
        var signalAtPosn = parseFloat(Math.round(signal[offsetIdx] * 100) / 100).toFixed(2);
        var posn = "[" + xAxisTickLabels[offsetIdx] + "] ▶ " + signalAtPosn;
        ...
        focus.attr("transform", "translate(" + xPosn + "," + (yPosn + 45) + ")");
        var bbox = focusText._groups[0][0].getBBox();
        var ctm = focusText._groups[0][0].getCTM();
        focus.selectAll('rect').remove();
        var rect = focus
