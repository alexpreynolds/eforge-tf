...
class TFTable extends React.Component {
  ...
  constructor(props) {
    ...
    this.convertTFOverlapsToReactTableDataObj = this.convertTFOverlapsToReactTableDataObj.bind(this);
...
//    console.log("tfTable - componentDidUpdate()", this.timeStampInMs);
    setTimeout(function() {
      //self.updateDimensions();
      $(".tfTable").css("max-height", parseFloat($("#viewer").height()) - parseFloat($("#plot-container").height()) - parseFloat($("#viewer-hint").height()) - parseFloat($("#viewer-gallery-header").height()) - 2*20.0);
    }, 100);
  }
...
*/
  ...
  convertTFOverlapsToReactTableDataObj(data) {
    ...
    for (let dbIdx = 0; dbIdx < dbsLength; dbIdx++) {
      ...
      tfOverlaps.forEach(function(element) {
        ...
        let position = element.chromosome + ":" + element.start + "-" + element.stop + ":" + element.strand;
  ...
  render() {
    ...
    let reactTableData = this.convertTFOverlapsToReactTableDataObj(probeTFOverlaps);
