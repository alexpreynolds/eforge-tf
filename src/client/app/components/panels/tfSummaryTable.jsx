...
class TFSummaryTable extends React.Component {
  ...
  exportGraph() {
    function convertToPDF(fName, summary) {
      ...
      var tf_summary_graph_query = "https://eforge-tf.altiusinstitute.org/assets/services/query_tf_summary_graph.py";
      axios({
          method: 'post',
    ...
    var date = new Date();
    var fName = ["eFORGE-TF", date.toISOString(), "pdf"].join('.');
  ...
  exportData() {
    ...
    var date = new Date();
    var fName = ["eFORGE-TF", date.toISOString(), "csv"].join('.');
    ...
    return (
      <div>
        <div className="tf-table-header" ref="tfTableHeader" id="tf-table-header">
            <p>Hypergeometric <em>p</em>-values are generated from counts of overlaps between a transcription factor and all probes of interest, and counts of overlaps between a transcription factor and all probes in the selected array.</p>
            <p>Multiple-testing correction of <strong>{totalNumberOfTests}</strong> <em>p</em>-value tests (<strong>{totalNumberOfTestsWithOneOrMoreOverlap}</strong> test results displayed, where there is one or more overlaps between probe and putative TF binding site) is performed with the Benjamini-Yekutieli method (FDR {fdrPct}%) to yield <em>q</em>-values.</p>
        </div>  
